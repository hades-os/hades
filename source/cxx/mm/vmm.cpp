...
void vmm::init() {
    ...
    boot_ctx = frg::construct<vmm_ctx>(memory::mm::heap);
    boot_ctx->page_map = new_pagemap();
    boot_ctx->create_hole((void *) 0x100000, 0x7ffffff00000);

    for (size_t i = 0; i < 8; i++) {
        void *phys = (void *) (i * memory::page_large);
        void *addr = (void *) (memory::x86::kernelBase + (i * memory::page_large));
        map_single_2m(phys, addr, page_flags::PRESENT, boot_ctx->page_map);
    }
    ...
    if (memory::pmm::nr_pages * memory::page_size < limit_4g) {
        for (size_t i = 0; i < limit_4g / memory::page_large; i++) {
            ...
            map_single_2m(phys, addr, page_flags::PRESENT | page_flags::WRITE, boot_ctx->page_map);
        }
    } else {
