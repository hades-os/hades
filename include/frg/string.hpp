...
public:
	...
	basic_string_view sub_string(size_t from) {
		size_t size = _length - from;
...
public:
	...
	bool startswith(const basic_string_view<Char>& other) const {
		if (_length < other.size()) {
			return false;
		}
	...
	bool eq(const basic_string_view<Char>& other) const {
		if (_length != other.size()) {
			return false;
		}
